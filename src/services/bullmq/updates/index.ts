...
import { incomingFolders, incomingUpdatesPath, processedUpdatesPath, UpdateTypeLookup, VersionState } from '~/shared/update';
import * as BullMQ from 'bullmq';
import _ from 'lodash';
import moment from 'moment';
import fs, { promises as fsAsync, mkdirSync } from 'fs';
...
export const UpdatesQueueName = process.env.UPDATES_QUEUE_NAME ?? "updatesQueueDev";
export const UpdatesQueue = new BullMQ.Queue<UpdateJobData>(
    UpdatesQueueName,
    {
...
const UseSourceLocalStorage = (process.env.UPDATE_SOURCE_PROVIDER || 'local') == 'local';
const UseOutputLocalStorage = (process.env.UPDATE_OUTPUT_PROVIDER || 'local') == 'local';
const SourceLocalStorageDir = process.env.UPDATE_SOURCE_DIRECTORY || '../updates-in';
const OutputLocalStorageDir = process.env.UPDATE_OUTPUT_DIRECTORY || '../updates-out';
...
const deleteDirectory = async (path: string) => {
    ...
    try {
        ...
        throw new Error(`Failed : failed to clean output path (${path})|(${error})`);
    }
}
...
const enumerateFiles = async (path: string, relativePath: string = ''): Promise<IFileInfo[]> => {
    ...
    for (const dirent of dirents) {
        if (dirent.isDirectory()) {
            files.push(...await enumerateFiles(path + dirent.name + '/', relativePath + dirent.name + '/'));
        }
    }
...
async function unzipFile(filename: string, outputDir: string)
{
    try {
        ...
        for (const filePath of files)
        {
            ...
            const data = Buffer.from(await fs.readFilePromise(filePath, 'base64'), 'base64');
...
const reportProcessFiles = (job: BullMQ.Job, processedCount: number, filesCount: number, progress: number[]) => {
    job.updateProgress(fixedProgress(progress[0] + progress[1] * (processedCount / filesCount)));
}
...
const processUpdateJob = async (job: BullMQ.Job<UpdateJobData>) => {
    ...
    try {
        ...
        const filesList: Array<IFileInfo[]> = new Array<IFileInfo[]>(incomingFolders.length);
        let filesCount = 0;
        let processedCount = 0;
        let reportCounter = 0;
    
        for (let n = 0; n < incomingFolders.length; ++n) {
            filesList[n] = await enumerateFiles(incomingPath + incomingFolders[n] + '/');
        ...
        for (let n = 0; n < UpdateTypeLookup.length; ++n) {
            ...
            for (let j = 0; j < files.length; ++j) {
                ...
                const dest = path.join(processedPath, file.filename + file.extension).replace(/\\/g, '/');
                const destDir = dest.substring(0, dest.lastIndexOf('/'));
                await fsExtra.ensureDir(destDir);
                await fsWriteFileAsync(processedPath + file.filename + file.extension, compressedBuffer);
        ...
        if (UseOutputLocalStorage) {
            const outputDir = resolve(OutputLocalStorageDir, UpdatePrefixDir, uploadPath);
        ...
        for (let n = 0; n < UpdateTypeLookup.length; ++n) {
            ...
            files.push(
                ...categoryFiles.map<IMDBUpdateFile>(
                    (file) => ({
